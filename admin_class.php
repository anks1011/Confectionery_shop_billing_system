...
Class Action {
	...
	function save_order(){
		...
		if(empty($id)){
			...
			while($i == 0){
				...
				$chk = $this->db->query("SELECT * FROM bills where ref_no ='$ref_no' ");
				if($chk->num_rows <= 0){
					$i = 1;
				}
			}
			$data .= ", ref_no = '$ref_no' ";
			$save = $this->db->query("INSERT INTO bills set $data");
			if($save){
				$id = $this->db->insert_id;
			}
		}else{
			$save = $this->db->query("UPDATE bills set $data where id = $id");
		}
		if($save){
			$ids = array_filter($item_id);
			$ids = implode(',',$ids);
			if(!empty($ids))
			$this->db->query("DELETE FROM billed_items where bill_id = $id and id not ($ids) ");
		foreach($item_id as $k=>$v){
			...
			if(empty($v)){
				$this->db->query("INSERT INTO billed_items set $data");
			}else{
				$this->db->query("UPDATE billed_items set $data where id = $v");
			}
		}
		return $id;
		}
	...
	function delete_order(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM bills where id = ".$id);
		$delete2 = $this->db->query("DELETE FROM billed_items where bill_id = ".$id);
		if($delete){
			return 1;
		}
	}
